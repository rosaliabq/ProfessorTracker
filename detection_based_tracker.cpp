...
static inline vector<Rect> join_vectors(vector<Rect> v1, vector<Rect> v2, vector<Rect> v3) 
{
	vector<Rect> vOut;
	vOut.clear();
	vOut = v1;
	...
	{
		vOut.push_back(v2.at(i) );
	}
	for (int j = 0 ; j < v3.size() ; j++ )
	{
		vOut.push_back(v3.at(j) );
	}
	return vOut;
}
...
public:
	...
	bool communicateWithDetectingThread(const Mat& imageGray, const Mat& mask, vector<Rect>& rectsWhereRegions);
	bool run();
	void stop();
	void resetTracking();
...
bool DetectionBasedTracker::SeparateDetectionWork::communicateWithDetectingThread(const Mat& imageGray, const Mat& mask, vector<Rect>& rectsWhereRegions)
{
	static double freq = getTickFrequency();
...
void DetectionBasedTracker::process(const Mat& imageGray, const Mat& mask)
{
	...
	bool shouldHandleResult=separateDetectionWork->communicateWithDetectingThread(imageGray, mask, rectsWhereRegions);
	...
	for(size_t i=0; i < rectsWhereRegions.size(); i++) {
		...
		detectInRegion(imageDetect, mask, r, detectedObjectsInRegions);
	}
...
void DetectionBasedTracker::detectInRegion(const Mat& img, const Mat& mask, const Rect& r, vector<Rect>& detectedObjectsInRegions)
{
